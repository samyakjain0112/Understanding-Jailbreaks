...
class Block(nn.Module):
    ...
    def forward(self, x, mask):
        ...
        return x, out_out, out_act_out, out_return
...
class GPT(nn.Module):
    ...
    def forward(self, idx, targets=None, mask=None, attack=None, emb=[],is_dpo=False,return_value=1):    
        device = idx.device
        if attack=='emb_in':
        ...
        for block in self.transformer.h:
            x, att_mat, att_mat_act, out_return = block(x, mask)
            #print(x.shape)
        ...
        if targets is not None:
            ...
            #print(loss)
        if return_value==1:
            return logits, lst_atten, lst_atten_act,   lst_atten_residual, lst_atten_copy,  loss
        else:
            return logits,  loss
