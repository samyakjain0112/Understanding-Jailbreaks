    ...
    def run_pretrain(self):
        ...
        while True:
            ...
            if self.config.is_dpo==1:
                ...
                logits_dpo, self.loss_safe_dpo = config.dpo_model(x,y,mask,reduction=True)
                ...
                logits_unsafe_dpo, self.loss_unsafe_dpo = config.dpo_model(x_ex,y_ex,mask,reduction=True)
                ...
                self.loss = -torch.mean(torch.log(torch.special.expit((self.loss_safe_dpo.detach()-self.loss_safe)*self.config.dpo_weight_safe - (self.loss_unsafe_dpo.detach()-self.loss_unsafe)*self.config.dpo_weight_unsafe)))
